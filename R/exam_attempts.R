...
#' @param opens,closes the opening and closing time of the exam for the given users,
...
parse_attempt_settings <- function (opens, closes, max_attempts, timelimit, grace_period) {
  ...
  list(opens = parse_datetime(opens),
...
initialize_attempt <- function (exam_id, exam_version) {
  session <- getDefaultReactiveDomain()
  session_env <- get_session_env(session)
  if (!is.null(session_env$attempt)) {
    # This is theoretically impossible.
    abort("An attempt is already initialized.")
  }
  ...
  # Create a reactive value indicating if a valid attempt is in progress
  session_env$attempt_status <- reactiveVal(NA)
  ...
  if (length(valid_attempts) > 0L) {
    ...
    session_env$attempt <- prev_attempts[[valid_attempts[[1L]]]]
  } else {
    ...
    session_env$attempt <- attempt
  }

  # Set the current attempt as valid
  session_env$attempt_status(TRUE)
  examinr_ns <- getNamespace('examinr')
  # Determine when attempt needs to be finished
  if (config$timelimit < Inf) {
    attempt_timeout <- min(as.numeric(session_env$attempt$started_at) + config$timelimit, as.numeric(config$closes),
                           na.rm = TRUE)
  ...
  # Update the client at every new section or when the attempt becomes invalid
  # Make handler independent of current context
  observe(expr({
    current_section <- get_current_section()
    status <- get_session_env()$attempt_status()
    if (!isTRUE(status == 'user_finished')) {
...
get_current_attempt <- function (session) {
  ...
  return(get_session_env(session)$attempt)
}

#' @importFrom shiny getDefaultReactiveDomain
is_attempt_active <- function (session) {
  if (missing(session)) {
    session <- getDefaultReactiveDomain()
  }
  return(isTRUE(get_session_env(session)$attempt_status()))
}
...
finish_current_attempt <- function (session, timeout = FALSE) {
  ...
  current_attempt <- session_env$attempt
  reason <- if (isTRUE(timeout)) { 'timeout' } else { 'user_finished' }

  # Only continue if the attempt is still active.
  if (!is.null(current_attempt) && isTRUE(isolate(session_env$attempt_status()))) {
    # First invalidate the attempt.
    session_env$attempt_status(reason)

    # Then save the data for the current section(s), but don't try to finish the attempt again.
    if (isTRUE(timeout)) {
      current_section <- isolate(session_env$section_state$current)
      if (!isTRUE(current_section)) {
        moduleServer(current_section$ui_id, session = session, function (input, output, section_session) {
