...
#' This data provider is called every time a section is rendered and should return the static as well as
#' the user-specific randomized values.
#'
...
#' @importFrom rlang warn
get_exercise_user_env <- function (exercise_label, session) {
  # get a "random seed" to ensure data generation does not determine all other r code
  reset_seed <- sample.int(.Machine$integer.max, 1L)
...
exercise_options <- function (title, button, button_context, timelimit, lines, autocomplete, df_print, points,
  ...
  global_exercise_options <- list(
    ...
    autocomplete = isTRUE(autocomplete %||% opts_chunk$get('exercise.autocomplete') %||% FALSE),
