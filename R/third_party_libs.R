...
cdn_script_tag <- function (libref, async = TRUE) {
  vapply(libref$scripts, FUN.VALUE = character(1L), function (script) {
    sprintf('<script %s src="%s/%s" integrity="%s" data-external="1" crossorigin="anonymous"></script>',
            if (async) 'async' else '', libref$base_href, script$src, script$integrity)
  })
}
...
html_dependency_ace <- function (use_cdn) {
  ...
  if (isTRUE(use_cdn)) {
    script_tags <- cdn_script_tag(res)

    htmlDependency('ace', version = res$version, src = c(href = paste(res$base_href, res$version, sep = '/')),
