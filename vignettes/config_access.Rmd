...
Currently, examinr ships with only two authentication providers: `rsconnect_auth()`, which uses the RStudio Connect user information, and `ui_auth()` which shows a login screen when the learner loads the exam.

### Login screen

The `ui_aith()` authentication provider works on every shiny server.
Learners visiting the exam URL are prompted for a username and password, which have to be somehow given to the learners beforehand.
