...
(function () {
    ...
    module.exports = {
      init: function () {
        ...
        if (progressMsg) {
          ...
          const sectionProgress = progressMsg.section.replace('{section_nr}', '<span class="examinr-section-nr">1</span>').replace('{total_sections}', '<span class="examinr-total-sections">' + (config.totalSections || 1) + '</span>');
          const timerHtml = progressMsg.timer.replace('{time_left}', '<span class="examinr-timer" role="timer">' + '<span class="hrs">??</span>' + '<span class="min">??</span>' + '<span class="sec"></span>' + '</span>');
        ...
        if (config.progressbar) {
          const progressbarEl = $('<div class="progress">' + '<div class="progress-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="' + config.totalSections + '" style="min-width: 2em;">' + '</div>' + '</div>');
    ...
    function initializeEditorBindings() {
        ...
        subscribe: function (exercise, callback) {
          ...
          exercise.find('.examinr-run-button').on('click.examinrExerciseInputBinding', function (event, forceSubmit) {
            if (exerciseRunButtonEnabled || forceSubmit === true) {
              ...
              exerciseRunning(exercise.find('.examinr-exercise-output'), exercise.data('evaluate') !== false);
              callback(true);
            }
          });
        },
      ...
      $.extend(outputBindings, {
        ...
        renderValue: function (exercise, data) {
          ...
          if (data.status) {
            const footerClass = data.status_class && data.status_class !== 'info' ? 'alert-' + data.status_class : 'text-muted';
  ...
  6: [function (require, module, exports) {
    ...
    function updateAttemptSelector(attempts, current) {
      ...
      if (sel.length > 0 && attempts && attempts.length > 0) {
        const options = attempts.map(at => '<option value="' + at.id + '" data-finishedat="' + (at.finishedAt || '') + '">' + (at.finishedAt ? utils.formatDate(at.finishedAt * 1000) : 'Unfinished (started at ' + utils.formatDate(at.startedAt * 1000) + ')') + '</option>').join('');
    ...
    Shiny.addCustomMessageHandler('__.examinr.__-feedback', function (data) {
      ...
      if (!centerEl) {
        ...
        $('main').show();
        sections.showAll();
        const rightInputGroup = status.append('<div class="input-group input-group-sm">' + '<div class="input-group-prepend">' + '<label class="input-group-text" for="' + attemptSelectorId + '">' + status.getMessage('feedback').attemptLabel + '</label>' + '</div>' + '<select class="custom-select" id="' + attemptSelectorId + '"></select>' + '</div>', 'right');
        ...
        if (data.grading) {
          ...
          if (data.users && data.users.length > 0) {
            ...
            userSel.prop('disabled', false).change(userChanged).append(data.users.map(user => '<option value="' + user.id + '">' + (user.displayName || user.id) + '</option>').join(''));
      ...
      if (!data.grading) {
        centerEl.html(status.getMessage('feedback').status.replace('{awarded_points}', awardedPoints).replace('{total_points}', totalPoints));
      }
      status.fixMainOffset();
    });
    ...
    function renderDefaultFeedback(question, feedback, grading) {
      ...
      if (feedback.solution) {
        footer.append('<div class="examinr-grading-feedback">' + '<h6>' + status.getMessage('feedback').solutionLabel + '</h6>' + '<div>' + feedback.solution + '</div>' + '</div>');
      }
      if (grading) {
        ...
        if (badge.find('.examinr-points-awarded').length === 0) {
          ...
          badge.html('<span class="examinr-points-awarded"></span><span class="sr-only"> out of </span> ' + '<span class="examinr-points-outof">' + outof + '</span>');
        }
        if (feedback.points || feedback.points === 0) {
          const context = feedback.points <= 0 ? 'danger' : feedback.points >= feedback.maxPoints ? 'success' : 'info';
          badge.removeClass('badge-secondary badge-info badge-success badge-danger').addClass('badge-' + context).find('.examinr-points-awarded').addClass('lead').text(feedback.points);
        } else {
          badge.removeClass('badge-secondary badge-info badge-success badge-danger').addClass('badge-secondary').find('.examinr-points-awarded').removeClass('lead').html('&mdash;');
        }
      }
      if (grading) {
        if (feedback.comment) {
        ...
        footer.append('<div class="text-muted examinr-grading-feedback">' + '<h6>' + status.getMessage('feedback').commentLabel + '</h6>' + '<div>' + feedback.commentHtml || feedback.comment + '</div>' + '</div>');
      }
    ...
    feedbackRenderer.push({
      ...
      callback: function (question, feedback, grading) {
        ...
        if (solution) {
          footer.append('<div class="examinr-grading-feedback">' + '<h5>' + status.getMessage('feedback').solutionLabel + '</h5>' + solution + '</div>');
        }
      }
    });
    module.exports = {};
  ...
  7: [function (require, module, exports) {
    ...
    function toggleFeedback() {
      ...
      if (gradingData.feedbackShown) {
        ...
        footer.append('<div class="input-group examinr-grading-feedback">' + '<div class="input-group-prepend">' + '<label class="input-group-text" for="' + commentInputId + '">' + status.getMessage('feedback').commentLabel + '</label>' + '</div>' + '<textarea class="form-control" id="' + commentInputId + '">' + (gradingData.feedback || '') + '</textarea>' + '</div>');
    ...
    function prepareQuestion() {
      ...
      const pointsInput = $('<input type="number" class="form-control" />').attr('max', gradingData.maxPoints).attr('id', utils.randomId());
    ...
    function checkAnswers(event) {
      ...
      if (missing.length > 0) {
        ...
        if (missing.find('.card-footer').length === 0) {
          missing.append('<div class="card-footer examinr-mandatory-message" role="alert">' + status.getMessage('sections').mandatoryError + '</div>');
        } else {
          missing.find('.card-footer').append('<div class="examinr-mandatory-message" role="alert">' + '<hr class="m-3" />' + status.getMessage('sections').mandatoryError + '</div>');
        }
      ...
      init: function () {
        ...
        actualSections.each(function () {
          ...
          if (sectionsConfig.progressive) {
            section.find('.examinr-section-next').on("click", {
              context: section
            }, checkAnswers);
          } else {
            section.find('.examinr-section-next').on("click", {
  ...
  11: [function (require, module, exports) {
    ...
    const dialogContainerTitle = $('<h4 class="modal-title" id="' + utils.randomId('examinr-status-dialog-title-') + '">');
    const dialogContainerContent = $('<div class="modal-body" id="' + utils.randomId('examinr-status-dialog-body-') + '">');
    const dialogContainerFooter = $('<div class="modal-footer">' + '<button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>' + '</div>');
    const dialogContainer = $('<div class="modal" tabindex="-1" role="alertdialog" ' + 'aria-labelledby="' + dialogContainerTitle.attr('id') + '"' + 'aria-describedby="' + dialogContainerContent.attr('id') + '">' + '<div class="modal-dialog modal-lg" role="document">' + '<div class="modal-content"><div class="modal-header"></div></div>' + '</div>' + '</div>');
...
/*!
  ...
  */
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('jquery'), require('./util.js')) : typeof define === 'function' && define.amd ? define(['jquery', './util.js'], factory) : (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Modal = factory(global.jQuery, global.Util));
